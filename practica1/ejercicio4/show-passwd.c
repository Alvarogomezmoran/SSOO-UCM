...
void copy_file(const char* src, const char* dest) {
    ...
    int src_fd = open(src, O_RDONLY | O_CREAT);
    if (src_fd < 0) {
...
int main(int argc, char *argv[])
{
	...
	options.infile = NULL; // Nueva opción para el archivo de entrada

	/* Parse command-line options */
	while((opt = getopt(argc, argv, "hvi:o:pc")) != -1) { // Agregar 'i' como opción
		switch(opt) {
		case 'h':
		...
		case 'c':
			...
			break;
		case 'o':
			if ((options.outfile = fopen(optarg, "w")) == NULL) {
				fprintf(stderr, "The output file %s could not be opened: ", optarg);
				perror(NULL);
				exit(EXIT_FAILURE);
			}
			break;
		case 'i': // Nueva opción para el archivo de entrada
	...
	// Si se especifica un archivo de entrada alternativo, copiar /etc/passwd
	if (options.infile) {
		if (access("/etc/passwd", R_OK) == 0) {
			copy_file("/etc/passwd", options.infile);
			
		} else {
			fprintf(stderr, "Error: El archivo /etc/passwd no existe o no es accesible.\n");
			exit(EXIT_FAILURE);
		}
	}
